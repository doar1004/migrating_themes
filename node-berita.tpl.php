...
<?php endif; ?>
  ...
  <div class="content">
	...
	<!-- AddThis Button END -->
  	...
  	<?php
  		$nid = $node->nid;
		$view_result = views_get_view_result("berita_argument");
		foreach($view_result as $item){
			if ($item->nid == $nid) {
				global $base_url; global $base_path;
				$featured_image = field_file_load($item->node_data_field_berita_terbaru_gambar_field_berita_terbaru_gambar_fid);
				...
				echo "<div class='ctitle'>".$item->node_title."<div class='ctgl'>,&nbsp;".$d."&nbsp;".$m."</div></div>";
				echo "<img src='".$featured_image."' />";
				echo $item->node_revisions_body;
				
				if (!empty($item->node_data_field_berita_terbaru_gambar_field_berita_terbaru_youtube_value)) {
